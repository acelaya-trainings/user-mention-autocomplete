...
const HighlightText: FC<{ text: string, filteringText: string }> = ({ text, filteringText }) => (
  <>
    {text.split(filteringText).map(
      (part, index, array) => array.length === index + 1 ? part : <>{part} <span className="comment-widget-users-dropdown__list-item--highlight">{filteringText}</span></>
    )}
  </>
)
  ...
  return (
    <>
      {[
        ...
        <div key={`${user.username}_${index}`} className="comment-widget-users-dropdown__list-item">
